...
box(1.33,2.62,.11,0x456760,0,1.31,0);for(const x of [-.70,.70])box(.085,2.73,.24,C.rust,x,1.365,.0,'metal');box(1.48,.09,.24,C.rust,0,2.69,0,'metal');
for(const y of [.42,1.36,2.25]){box(.16,.22,.065,C.rust,-.63,y,.10,'metal');for(const dy of [-.055,.055])bolts(-.62,y+dy,.143);tube([[-.71,y-.10,.1],[-.71,y+.1,.1]],.026,C.gold,'metal');}
for(let i=0;i<6;i++)box(.65,.035,.035,C.ink,0,2.00+i*.068,.075);box(1.10,.15,.055,C.rust,0,.38,.075,'metal');
for(const x of [-.36,0,.36]){ball(.076,.076,.022,C.gold,x,1.22,.10,'metal');tube([[x,1.22,.12],[x,1.03,.22]],.021,C.rust,'metal');ball(.043,.062,.043,C.red,x,1.03,.22);}
for(let i=0;i<15;i++)chip(Math.sin(i*6)*.55,.12+Math.abs(Math.cos(i*4))*2.31,.063,.028+(i%3)*.009);return finish();}
