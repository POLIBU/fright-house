export default function(THREE){
 ...
 box(4.2,3.2,.14,0x363c4e,0,1.6,0);
 for(const side of [-1,1]){const before=new Set(g.children);
  shape([[-1.1,.10],[-1.1,2.17],[0,3.01],[1.1,2.17],[1.1,.10]],C.rust,.09,.065,.025);
  shape([[-.97,.20],[-.97,2.12],[0,2.86],[.97,2.12],[.97,.20]],0x70867b,.17,.02,.025);
  for(let j=0;j<9;j++)box(.012,1.80,.012,0x3b554f,-.85+j*.21,1.18,.204);
  for(const x of [-1.77,1.77]){box(.14,3.20,.20,C.gold,x,1.6,0);for(const y of [.4,1.6,2.8])bolts(x,y,.12);}
  for(const y of [.45,.85]){box(1.84,.07,.075,C.rust,0,y,.24,'metal');for(const x of [-.80,.8])bolts(x,y,.29);}
  for(const x of [-.62,0,.62]){ball(.056,.056,.025,C.gold,x,.70,.29,'metal');box(.025,.20,.055,C.gold,x,.62,.30,'metal');}
