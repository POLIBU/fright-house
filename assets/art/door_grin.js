export default function(THREE){
 ...
 box(4.2,3.2,.14,0x6e302e,0,1.6,0);for(const side of [-1,1]){const z=.095;
  const front=new THREE.Group();g.add(front);const before=new Set(g.children);
  shape([[-1.16,.12],[-1.16,2.27],[-1.13,2.84,1.13,2.84,1.16,2.27],[1.16,.12]],C.gold,z,.035,.02);
  shape([[-1.01,.20],[-1.01,2.23],[-.98,2.68,.98,2.68,1.01,2.23],[1.01,.20]],0x314c47,z+.05,.025,.018);
  for(const x of [-.92,.92])box(.055,2.05,.035,C.gold,x,1.24,z+.1);box(1.86,.055,.035,C.gold,0,.25,z+.1);
  for(let i=0;i<9;i++){const a=Math.PI*i/8;ball(.037,.037,.027,i%3?0xc9ab60:0x744d33,Math.cos(a)*1.06,2.27+Math.sin(a)*.40,z+.12,'glass');}
  for(const x of [-1.56,1.56]){box(.13,2.94,.10,C.gold,x,1.58,z);for(let i=0;i<6;i++)ball(.032,.032,.012,C.gold,x,.32+i*.49,z+.07,'metal');}
  for(const x of [-.73,.73]){box(.07,.24,.05,C.gold,x,.70,z+.14,'metal');rim(x,.69,.09,.11,C.gold,z+.19,.11);}
  if(side<0){for(const o of [...g.children])if(!before.has(o)&&o!==front){o.position.z=-o.position.z;o.rotation.y+=Math.PI;}}
 }
