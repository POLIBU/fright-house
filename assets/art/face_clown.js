export default function(THREE){
 ...
 head(0xcbbc8f,[ellipse(-.22,.23,.17,.21),ellipse(.22,.23,.17,.21),ellipse(0,-.28,.38,.23)],.60,.69);
 for(const s of [-1,1]){eye(s*.22,.23,.17,.21,{color:0x486e65});brow(s*.22,.49,.17,C.ink,s);cheek(s*.40,-.10,C.red,.14,.13);ball(.13,.23,.12,C.red,s*.61,.12,.12);for(let j=0;j<3;j++)ball(.09,.12,.07,C.rust,s*(.6+j*.035),.20+j*.12,.10);}
 ball(.145,.125,.18,C.red,0,.035,.35,'glass');mouth(0,-.29,.38,.23,C.red);for(let i=-3;i<=3;i++)tooth(i*.088,-.20-Math.abs(i)*.018,.078,.105);ball(.16,.043,.038,C.pink,.04,-.42,.25);
 for(let i=-3;i<=3;i++){const o=box(.13,.18,.10,i%2?C.gold:C.bone,i*.12,-.74+Math.abs(i)*.02,.10);o.rotation.z=i*.14;}
 crack([[.0,.67,.28],[-.12,.57,.28],[-.07,.44,.28],[-.13,.34,.28]]);chip(.45,.39,.28,.08);chip(-.35,-.47,.28,.09);
