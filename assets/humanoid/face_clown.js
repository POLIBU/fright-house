export default function(THREE){
 const cfg={"title":"The Crooked Clown","width":0.49,"depth":0.285,"temple":0.05,"brow":0.066,"browTilt":0.03,"cheek":0.087,"hollow":0.075,"nose":1,"noseBend":0.05,"fold":0.021,"jowl":0.018,"age":3,"asym":0.018,"cant":0.026,"mouthWidth":0.285,"smile":0.12,"sneer":0.28,"gape":0.052,"lip":0.01,"eyeOpen":0.03,"shut":0,"missing":0,"gaze":0.003,"stray":0.014,"skin":13879208,"lips":7939876,"iris":5659476,"paint":3427168,"teeth":true,"missingTooth":2,"cracks":3,"staples":true};
 ...
 for(let i=0;i<p.count;i++){
  ...
  if(facing>.12){
   const tear=Math.abs(x-Math.sign(x)*(.18+.025*Math.sin(y*8)))<(.018+Math.max(0,.07-y)*.07)&&y<.15&&y>-.16;
   const browPaint=y>.25&&y<.47&&Math.abs(Math.abs(x)-.19)<(.47-y)*.40;
   if(tear||browPaint)c.lerp(new THREE.Color(0x263949),.84*facing);
   const rouge=gauss(Math.abs(x),y,.33,-.17,.066,.065);c.lerp(new THREE.Color(0x9a332e),rouge*.78*facing);
  }
 ...
 // Thin paint patches follow the sculpt, retaining saturated faded makeup under the torch.
 const bluePaint=new THREE.MeshStandardMaterial({color:0x263c4e,roughness:.91});
 function paintedPatch(points,material){
  const xs=points.map(p=>p[0]),ys=points.map(p=>p[1]),x0=Math.min(...xs),x1=Math.max(...xs),y0=Math.min(...ys),y1=Math.max(...ys);
  const geometry=new THREE.PlaneGeometry(x1-x0,y1-y0,24,32),a=geometry.attributes.position;
  for(let i=0;i<a.count;i++){const x=a.getX(i)+(x0+x1)/2,y=a.getY(i)+(y0+y1)/2;a.setXYZ(i,x,y,front(x,y)+.012);}
  function inside(x,y){let hit=false;for(let i=0,j=points.length-1;i<points.length;j=i++){const [xi,yi]=points[i],[xj,yj]=points[j];if((yi>y)!==(yj>y)&&x<(xj-xi)*(y-yi)/(yj-yi)+xi)hit=!hit;}return hit;}
  const old=geometry.index.array,kept=[];for(let i=0;i<old.length;i+=3){const ids=[old[i],old[i+1],old[i+2]],x=ids.reduce((s,n)=>s+a.getX(n),0)/3,y=ids.reduce((s,n)=>s+a.getY(n),0)/3;if(inside(x,y))kept.push(...ids);}geometry.setIndex(kept);geometry.computeVertexNormals();add(geometry,material);
 }
 for(const side of [-1,1]){
  paintedPatch([[side*.12,.25],[side*.20,.45],[side*.27,.26]],bluePaint);
  paintedPatch([[side*.135,.075],[side*.24,.068],[side*.215,-.14]],bluePaint);
  const cheek=[];for(let i=0;i<28;i++){const a=i/28*Math.PI*2;cheek.push([side*.35+Math.cos(a)*.058,-.15+Math.sin(a)*.050]);}paintedPatch(cheek,lipMat);
 }
 oval(.088,.077,.075,lipMat,-.018,-.122,front(-.018,-.122)+.045);
