export default function(THREE){
 ...
 head(0xbba997,[ellipse(-.23,.14,.18,.12),ellipse(.22,.19,.15,.18),ellipse(.03,-.31,.28,.20)],.53,.57);
 for(const s of [-1,1]){shape([[s*.04,.46],[s*.39,1.07,s*.85,.91,s*.75,.53],[s*.46,.79,s*.43,.48,s*.36,.37]],s<0?C.red:C.blue,.035,.11,.03);ball(.07,.07,.07,C.gold,s*.74,.53,.13,'metal');}
 eye(-.23,.14,.18,.12,{color:C.ink,pupil:false});eye(.22,.19,.15,.18,{color:C.ink});brow(-.23,.35,.20,C.ink);brow(.22,.43,.17,C.ink,-1);
 shape([[-.05,.21],[.10,.08],[.01,-.14],[-.08,.00]],C.gold,.25,.14,.025);mouth(.03,-.31,.28,.20,C.red);ball(.105,.16,.06,C.red,.14,-.43,.27);tooth(-.10,-.23,.08,.11);tooth(.01,-.20,.08,.10);
 for(let i=-2;i<=2;i++)shape([[i*.18-.12,-.52],[i*.18,-.86+Math.abs(i)*.08],[i*.18+.12,-.52]],i%2?C.blue:C.red,.07,.05,.01);
 crack([[-.40,.32,.26],[-.30,.15,.28],[-.38,-.02,.28]]);chip(.38,-.29,.27,.08);
