...
const sh=new THREE.Shape();sh.moveTo(-2.1,0);sh.lineTo(2.1,0);sh.lineTo(2.1,3.08);sh.quadraticCurveTo(0,3.32,-2.1,3.08);sh.closePath();mesh(new THREE.ExtrudeGeometry(sh,{depth:.16,bevelEnabled:false}),0x244d4c,0,0,-.08);for(const z of [-.11,.11])for(let i=-3;i<=3;i++){const m=box(.44,.44,.03,0xbb8d42,i*.56,1.6,z);m.rotation.z=Math.PI/4;}for(const y of [.12,2.95])box(4.2,.1,.23,0xa87d41,0,y,0);
