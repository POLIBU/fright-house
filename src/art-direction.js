...
export function spiralMaterial(){
 ...
 material.onBeforeCompile=shader=>{
  ...
  shader.fragmentShader=shader.fragmentShader.replace('#include <map_fragment>',`#ifdef USE_MAP
   ...
   float family=mod(floor(vPanelSeed),6.0);
   vec2 q=meltedUv*2.0-1.0;
   float freq=7.0+mod(vPanelSeed,7.0),pattern=0.0;
   if(family<.5)pattern=sin(atan(q.y,q.x)*freq+length(q)*7.0);
   else if(family<1.5)pattern=sin((q.x+.13*sin(q.y*9.0))*freq)*sin((q.y+.10*sin(q.x*8.0))*freq);
   else if(family<2.5)pattern=sin((q.x+abs(fract(q.y*2.0)-.5)*1.7)*freq*2.0);
   else if(family<3.5)pattern=sin(length(q*vec2(1.0,1.5))*freq*3.0);
   else if(family<4.5)pattern=sin(max(abs(q.x),abs(q.y))*freq*3.0+atan(q.y,q.x)*2.0);
   else pattern=sin((q.x/(.35+abs(q.y)))*freq)*sin(log(.12+abs(q.y))*freq);
   vec3 ink=family<2.0?vec3(.42,.047,.035):family<4.0?vec3(.038,.14,.14):vec3(.11,.06,.16);
   sampledDiffuseColor.rgb=mix(ink,vec3(.69,.59,.38),smoothstep(-.06,.06,pattern))*(.84+sampledDiffuseColor.r*.18);
 ...
 material.customProgramCacheKey=()=> 'fright-optical-illusions-v3';
 return material;
}
export function addCircusCanopy(scene){
 ...
 for(let i=0;i<pos.count;i++){const x=pos.getX(i),z=pos.getY(i),radius=Math.hypot(x,z);const a=Math.atan2(z,x);const height=3.32+1.30*Math.max(0,1-radius/14.8)+.085*Math.sin(a*24)*Math.min(1,radius/4)-.14*Math.sin(radius*.9);pos.setXYZ(i,x+8.4,height+floorHeight(x+8.4,z+8.4),z+8.4);}const index=geo.index,kept=[];for(let i=0;i<index.count;i+=3){const a=index.getX(i),b=index.getX(i+1),c=index.getX(i+2),x=(pos.getX(a)+pos.getX(b)+pos.getX(c))/3,z=(pos.getZ(a)+pos.getZ(b)+pos.getZ(c))/3;if(Math.abs(x)<1.25&&Math.abs(z)<1.25)continue;kept.push(a,b,c);}geo.setIndex(kept);geo.computeVertexNormals();
 const cloth=new THREE.Mesh(geo,new THREE.MeshStandardMaterial({map:tex,roughness:1,side:THREE.DoubleSide}));cloth.receiveShadow=true;scene.add(cloth);const clothTime={value:0};cloth.userData.time=clothTime;cloth.material.onBeforeCompile=shader=>{shader.uniforms.clothTime=clothTime;shader.vertexShader='uniform float clothTime;\n'+shader.vertexShader;shader.vertexShader=shader.vertexShader.replace('#include <begin_vertex>','#include <begin_vertex>\ntransformed.y += .018*sin(position.x*1.4+clothTime*.65)*sin(position.z*.7+clothTime*.41);');};cloth.material.customProgramCacheKey=()=> 'breathing-circus-cloth';
...
export function addMirrors(scene){
 ...
 for(const s of specs){const root=new THREE.Group();root.position.set(s.x,floorHeight(s.x,s.z),s.z);root.rotation.y=s.rot;scene.add(root);
  ...
  const mirror=new Reflector(glassGeo,{color:0x999b92,textureWidth:touch?384:768,textureHeight:touch?768:1024,clipBias:.006,multisample:0,shader});mirror.camera.layers.enable(1);mirror.position.set(0,1.42,.055);mirror.userData={id:s.id,mode:s.mode,updates:0};root.add(mirror);mirrors.push(mirror);
