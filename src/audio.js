export class Soundscape{
 ...
 resetAtmosphere(){this.nextStep=0;this.nextBell=0;this.nextLaugh=12;this.nextMonsterLaugh=0;this.nextCart=0;this.nextGrunt=0;this.stop('monsterGrunt');this.stop('laughter');this.stop('monsterLaugh');}
 stop(name){if(name==='monsterGrunt'&&this.gruntNodes){for(const n of this.gruntNodes)n.disconnect();this.gruntNodes=null;}if(name==='monsterLaugh'&&this.monsterVoice){for(const n of this.monsterVoice.nodes)n.disconnect();this.monsterVoice=null;}const s=this.sources.get(name);if(s){try{s.stop();}catch{}this.sources.delete(name);}}
 ...
 grunt(t,distance,pan=0,blocked=false){
  if(!this.ctx||t<(this.nextGrunt||0)||this.sources.has('monsterGrunt')||this.sources.has('phoneVoice'))return;
  if(!this.gruntBuffer){const sr=this.ctx.sampleRate,b=this.ctx.createBuffer(1,Math.ceil(sr*1.65),sr),a=b.getChannelData(0);let phase=0,noise=0,seed=191;
   for(let i=0;i<a.length;i++){const u=i/sr,life=i/a.length;seed=(seed*1664525+1013904223)>>>0;noise=noise*.85+(seed/4294967296*2-1)*.15;phase+=(54-19*life+5*Math.sin(u*8))/sr*Math.PI*2;const voice=Math.sin(phase)+.32*Math.sin(phase*2.04)+.17*Math.sin(phase*3.1);const pulse=.72+.28*Math.sin(u*31);a[i]=Math.tanh(voice*1.9+noise*1.8)*Math.sin(Math.PI*life)**.65*pulse*.63;}this.gruntBuffer=b;
  }
  const source=this.ctx.createBufferSource(),filter=this.ctx.createBiquadFilter(),gain=this.ctx.createGain(),panner=this.ctx.createStereoPanner();source.buffer=this.gruntBuffer;source.playbackRate.value=.82+Math.random()*.16;filter.type='lowpass';filter.frequency.value=blocked?220:900;gain.gain.value=(.19+.82/(1+(distance/5)**2))*(blocked?.35:1);panner.pan.value=Math.max(-1,Math.min(1,pan));source.connect(filter).connect(gain).connect(panner).connect(this.master);this.gruntNodes=[source,filter,gain,panner];this.sources.set('monsterGrunt',source);source.start();this.nextGrunt=t+(distance<7?2.7:4.8)+Math.random()*1.5;source.onended=()=>{if(this.sources.get('monsterGrunt')===source)this.stop('monsterGrunt');};
 }
 monster(t,active,distance,pan=0,blocked=false){
  if(!active){this.stop('monsterLaugh');this.stop('monsterGrunt');return;}
  if(!this.ctx)return;
  this.grunt(t,distance,pan,blocked);
