...
export const ACTIVE_NODES=new Set(Array.from({length:N*N},(_,i)=>key(i%N,Math.floor(i/N))).filter(n=>!['1,0','3,1','3,4'].includes(n)));
for(const e of edges)if(e.split('|').some(n=>!ACTIVE_NODES.has(n)))edges.delete(e);
...
export function pathfind(from,to,states=INITIAL_GATES){if(!ACTIVE_NODES.has(from)||!ACTIVE_NODES.has(to))return [];const q=[from],prev=new Map([[from,null]]);for(let i=0;i<q.length;i++){const a=q[i];if(a===to){const p=[];let c=a;while(c!==null){p.unshift(c);c=prev.get(c);}return p;}for(const b of neighbors(a,states)){if(!prev.has(b)){prev.set(b,a);q.push(b);}}}return [];}
...
// The puzzle labyrinth is intentionally level: all three rotating walls share one floor.
// Elevation and moving floors belong to the separate entrance/mezzanine sequence.
export function floorHeight(x,z){return 0;}
