...
export function newRide(progress=0){return {age:0,nextThrow:6,axe:null,axeHits:0,throws:0,stun:0,flashCooldown:0,flashLight:0,flashCount:0,hitRows:new Set(),progress,lateral:0,speed:0,gap:24,energy:1,slow:0,hits:0,hitFlash:0,passed:new Set(OBSTACLES.filter(o=>o.distance<progress).map(o=>o.distance)),status:'riding'};}
export function stepRide(s,dt,{steer=0,boost=false,brake=false,flash=false,lookBack=false}={}){
 ...
 s.stun=Math.max(0,s.stun-dt);s.flashCooldown=Math.max(0,s.flashCooldown-dt);s.flashLight=Math.max(0,s.flashLight-dt);
 if(flash&&lookBack&&s.flashCooldown===0){s.flashCooldown=7;s.flashLight=.20;s.flashCount++;s.stun=2.2;if(s.axe?.phase==='windup')s.axe=null;s.nextThrow=Math.max(s.nextThrow,s.age+8);}
 ...
 // Delay an attack if its impact would coincide with a constrained obstacle row.
 const predicted=s.progress+Math.max(6,s.speed)*2.9;
 if(!s.axe&&s.stun===0&&s.age>=s.nextThrow&&!OBSTACLES.some(o=>Math.abs(o.distance-predicted)<8)){s.axe={phase:'windup',age:0,lateral:Math.round(s.lateral/LANE_WIDTH)*LANE_WIDTH,start:0,target:0,hit:false};s.throws++;s.nextThrow=s.age+8.5;}
 if(s.axe){const a=s.axe,old=axePosition(s);a.age+=dt;if(a.phase==='windup'&&a.age>=1.1){a.phase='flight';a.age=0;a.start=s.progress-s.gap;a.target=s.progress+s.speed*1.8;}else if(a.phase==='flight'){const now=axePosition(s);if(!a.hit&&sweptContact({progress:before,lateral:previousLateral},{progress:s.progress,lateral:s.lateral},old,{...now,kind:'axe',halfWidth:.14,halfDepth:.20})){a.hit=true;s.axeHits++;s.hits++;s.slow=.9;s.hitFlash=.7;s.gap-=2;}if(a.age>2.25)s.axe=null;}}
 const monsterSpeed=s.stun>0?1.5:s.progress>260?8.25:7.15;
...
export function axePosition(s){const a=s.axe;if(!a)return null;const u=a.phase==='windup'?0:a.age/1.8;return {distance:a.phase==='windup'?s.progress-s.gap:a.start+(a.target-a.start)*u,lateral:a.lateral,height:1.15+Math.sin(Math.min(1,u)*Math.PI)*.85,angle:s.age*16};}
export function rideSnapshot(s){const next=OBSTACLES.find(o=>o.distance+2.3>s.progress);return {axe:s.axe?{...s.axe,...axePosition(s)}:null,axeHits:s.axeHits,throws:s.throws,stun:s.stun,flashCooldown:s.flashCooldown,flashLight:s.flashLight,flashCount:s.flashCount,age:s.age,progress:s.progress,length:RIDE_LENGTH,lateral:s.lateral,speed:s.speed,gap:s.gap,energy:s.energy,hits:s.hits,status:s.status,hazards:OBSTACLES.filter(o=>Math.abs(o.distance-s.progress)<32).flatMap(o=>hazardState(o,s.age)),next:next?{distance:next.distance-s.progress,lanes:next.lanes,kind:next.kind}:null};}
