...
export async function buildWorld(scene,{assetBase='./assets/'}={}){
 ...
 function mountFace(name,x,z,rotation,height=1.48){const f=art['face_'+name].clone();const scale=height/f.userData.nativeSize.y;f.scale.setScalar(scale);const d=f.userData.nativeSize.z*scale/2+.12;f.position.set(x+Math.sin(rotation)*d,.94,z+Math.cos(rotation)*d);f.rotation.y=rotation;staticRoot.add(f);}
 ...
 function setLighting(chase=false){lights.forEach((l,i)=>{l.color.setHex(chase?(i%4===0?0xb44a36:0x729f95):l.userData.baseColor);l.intensity=l.userData.baseIntensity*(chase?.75:1);l.userData.level=l.intensity;});}
 ...
 return {collision,gateMeshes,interactables,creature,fixtures,lights,jam,shutter,proto,mirrors,evidenceMeshes,setLighting,updateLighting};
}
