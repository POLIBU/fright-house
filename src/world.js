...
export async function buildWorld(scene){
 ...
 function wall(x,z,rot,idx){const o=place('panel',x,0,z,rot);wallMeshes.push(o);solid(x,z,rot? .20:SIZE,rot?SIZE:.20);
  if(idx%3!==1){for(const side of [-1,1]){const f=proto.face.clone();f.position.set(0,.84,side*.16);f.rotation.y=side<0?Math.PI:0;o.add(f);}}
 ...
 for(let z=0;z<N;z++)for(let x=0;x<N;x++){
  ...
  fixtures.push(bulb);
  if((x+z)%2===0){const l=new THREE.PointLight((x===4&&z>2)?0x72bbb1:0xffb963,17,8,2);l.position.set(x*SIZE,2.7,z*SIZE);scene.add(l);}
 }
 GATES.forEach((g,i)=>{const pivot=new THREE.Group();pivot.position.set((g.x+.5)*SIZE,0,(g.z+.5)*SIZE);scene.add(pivot);const panel=proto.panel.clone();panel.position.x=-SIZE/2;pivot.add(panel);for(const side of [-1,1]){const f=proto.face.clone();f.position.set(-SIZE/2,.84,side*.13);f.rotation.y=side<0?Math.PI:0;pivot.add(f);}gateMeshes.push(pivot);
  const p=point(g.a);const stand=new THREE.Group();stand.position.set(p.x-1.25,0,p.z-1.25);scene.add(stand);box(.14,1.16,.14,0x3e5145,0,.58,0,stand);const wheel=new THREE.Mesh(new THREE.TorusGeometry(.18,.025,8,20),new THREE.MeshStandardMaterial({color:0xa88958,metalness:.6,roughness:.45}));wheel.position.y=1.22;stand.add(wheel);const tag=label(g.name,1.10,.22);tag.position.set(0,1.61,0);stand.add(tag);const stat=label('OPEN EAST',.92,.20);stat.position.set(0,.94,.04);stand.add(stat);const compass=label('N ↑   S ↓',1.2,.3,'#b29c65','#222b24');compass.rotation.x=-Math.PI/2;compass.position.set(p.x,.012,p.z-1.3);scene.add(compass);
 ...
 for(const e of EVIDENCE){const p=point(e.node);box(1,.075,.65,0x574b34,p.x,1.03,p.z+.72);for(const a of [-.40,.40])box(.07,1,.5,0x263e36,p.x+a,.5,p.z+.72);solid(p.x,p.z+.72,1,.65,1.05);const obj=place(e.asset,p.x,1.07,p.z+.72,Math.PI);if(e.id==='tape')obj.scale.setScalar(2.2);const tag=label(e.id==='bag'?'LOST PROPERTY':e.id==='shoe'?'THE SUN ROOM':'LAST PERFORMANCE',2,.3);tag.position.set(p.x,2.35,p.z+1.9);tag.rotation.y=Math.PI;staticRoot.add(tag);interactables.push({id:e.id,type:'evidence',position:new THREE.Vector3(p.x,1.28,p.z+.72),mesh:obj,name:e.name});}
 ...
 return {collision,gateMeshes,interactables,creature,fixtures,jam,shutter,proto};
}
