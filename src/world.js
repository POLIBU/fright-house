...
export async function buildWorld(scene,{assetBase='./assets/'}={}){
 ...
 const atmosphereState=()=>({faces:floatingFaces.length,teddyEyes:teddy.state(),fishBags:attractions.bags,webs:2,skeletons:1,faceY:floatingFaces[0].mesh.position.y,balloons:balloons.items.length,balloonY:balloons.items[0].position.y,melt:hauntLevel,meltAge,lightScale:haunting?.55-.34*hauntLevel:1,arms:rig.arms.length,reach:creature.userData.reach,smoke:creature.visible?rig.smoke.children.length:0});
