...
for(let level=1;level<=11;level++){
 await p.goto('http://localhost:8097/two-d/'+(level===1?'':`level-${level}.html`));await p.locator('#start:not([disabled])').waitFor({state:'attached'});if(level===1)await p.locator('#start').tap();else await p.locator('header span').tap();
