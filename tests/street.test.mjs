...
test('wheel and bulb animation are deterministic and continue over time',()=>{assert.notEqual(wheelAngle(1),wheelAngle(5));assert.notEqual(bulbLevel(1,0),bulbLevel(1.7,0));assert.equal(bulbLevel(0,1,true),bulbLevel(9,1,true));const s=newStreet();tick(s,1);assert.equal(s.time,1);});
