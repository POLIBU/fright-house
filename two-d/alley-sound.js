...
export function createAlleySound(ac,output){
 ...
 function tick(s,muted=false){const active=['unload','alley'].includes(s.phase),travel=previous?Math.hypot(s.x-previous.x,s.y-previous.y):0;previous={x:s.x,y:s.y};
  ...
  if(s.moving&&travel>0&&travel<30){distance+=travel;if(distance>=27&&play('step',.6,counts.step%2?.12:-.12,.96+(counts.step%3)*.035))distance%=27;}else distance=22;
  if(s.leafFlights>lastLeaves&&s.time>=nextLeaves){if(play('leaves',.38,Math.max(-.5,Math.min(.5,(s.x-240)/100)),.94+(counts.leaves%3)*.06))nextLeaves=s.time+.55;}lastLeaves=s.leafFlights;
