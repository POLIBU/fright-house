...
export function tickClown(s,dt){const c=s.clown;if(!c||!s.power)return;c.moving=false;if(s.powerAge<.9)return;if(s.powerAge<3){const x=835-(s.powerAge-.9)*30;c.moving=true;c.walk+=dt*9;c.face='left';c.x=x;c.y=661+Math.min(1,(s.powerAge-.9)/1.2)*(floors[0]-661);return;}
