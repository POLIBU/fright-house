...
export function tickEpilogue(s,input,dt){if(s.paused||s.phase==='complete')return;dt=clamp(dt,0,.05);s.time+=dt;s.age+=dt;s.moving=false;
 ...
 else if(s.phase==='alley'){let dx=Number(!!input.right)-Number(!!input.left),dy=Number(!!input.down)-Number(!!input.up);if(!dx&&!dy&&input.target){dx=input.target.x-s.x;dy=input.target.y-s.y;if(Math.hypot(dx,dy)<3)dx=dy=0;}const n=Math.hypot(dx,dy);if(n){const step=Math.min(95*dt,input.target&&!input.up&&!input.down&&!input.left&&!input.right?n:95*dt);s.x=clamp(s.x+dx/n*step,ALLEY.minX,ALLEY.maxX);s.y=clamp(s.y+dy/n*step,ALLEY.endY,1260);if(s.y<190&&dy<0)s.x+=(240-s.x)*Math.min(1,dt*5);s.moving=true;s.walk+=dt*10;s.face=Math.abs(dx)>Math.abs(dy)?dx<0?'left':'right':dy<0?'up':'down';}s.dawn=Math.max(s.dawn,clamp((ALLEY.startY-s.y)/(ALLEY.startY-ALLEY.endY),0,1));if(s.y<ALLEY.tapeY+8&&!s.tapeBroken){s.tapeBroken=true;s.tapeAge=0;s.notice='Police tape. Someone else has been here.';}if(s.y<=ALLEY.endY+1){phase(s,s.branch==='arrest'?'approach':'escape');}}
