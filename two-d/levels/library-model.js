...
export function libraryAction(s,id){if(s.paused||s.inspection||['caught','escaped','breaking'].includes(s.phase))return null;const o=id?LIBRARY_OBJECTS.find(o=>o.id===id&&Math.hypot(o.x-s.x,o.y-s.y)<o.r):libraryNear(s);if((!id||id==='book')&&s.book.landed&&!s.book.collected&&Math.hypot(s.x-s.book.x,s.y-s.book.y)<35){s.book.collected=true;s.notice='You snatch the maintenance book. PLATFORM III instructions saved. The exit latch releases.';s.noticeAge=5;return 'book';}if(!o)return null;if(o.id==='phone'){s.inspection=true;return 'phone';}if(o.id==='specimens'){if(s.phoneAnswered&&s.phase==='search'){s.phase='breaking';s.phaseAge=0;s.notice='Your sleeve catches the shelf. Glass slips against wood…';s.noticeAge=3;return 'jar';}s.notice='The telephone will not stop ringing.';s.noticeAge=3;}if(o.id==='exit'){if(s.phase==='chase'&&!s.book.collected){s.notice='The exit is interlocked with the fallen maintenance book. Take it from the floor!';s.noticeAge=4;return 'locked';}if(s.phase==='chase'&&!s.unlocking&&!s.openingExit){s.unlocking=true;s.unlockAge=0;s.notice='The lock is grinding open. Dodge its bile until the bolt releases!';s.noticeAge=4;return 'exit';}s.notice=s.unlocking?'The bolt is still turning — keep moving!':'The exit is locked. The desk telephone is still ringing.';s.noticeAge=3;}return o.id;}
