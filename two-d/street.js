...
const toyInspector=createToyInspector({isMuted:()=>muted,onExplode(){s.toyBroken=true;s.toy=true;persist();},onClose(){dialog=null;release();},onDiscover(){s.toy=true;remember(campaign,'windup-mark','A wind-up carousel concealed a brass plate marked 03: maintenance access, upper attraction.');persist();}});
...
function interact(targetId=null){if(!running||paused||s.phase==='entering')return;if(dialog){dismiss();return;}const o=typeof targetId==='string'?INTERACTIONS.find(o=>o.id===targetId&&Math.hypot(o.x-s.x,o.y-s.y)<o.r):near(s);if(!o)return;release();
  ...
  if(o.id==='toy'){dialog={name:'WIND-UP CAROUSEL',type:'3d'};toyInspector.open(s.toy,'toy',!!s.toyBroken);}
...
function render(){ctx.clearRect(0,0,480,360);if(ready)ctx.drawImage(bg,0,0,480,360);ferris();forecourt?.draw(ctx,s,false);lighting();props.draw(ctx,'ticket');props.draw(ctx,'toy',s.toyBroken?1:0);if(s.y>=150)props.draw(ctx,'gate',s.gateProgress);if(running||s.phase==='complete'){ctx.save();if(s.phase==='entering')ctx.globalAlpha=Math.max(0,1-s.entryTime/.65);drawPlayer(ctx,s,s.moving&&!paused&&!dialog);if(s.phase!=='entering')flashlight();ctx.restore();}forecourt?.draw(ctx,s,true);paper();if(s.phase==='entering'){props.draw(ctx,'mouth',mouthProgress(s.entryTime));}props.drawRavens(ctx,s.ravens);if(s.y<150)props.draw(ctx,'gate',s.gateProgress);fogLayer();
...
function frame(now){const dt=Math.min(.04,(now-last)/1000);last=now;if(running&&!paused&&!dialog){if(s.phase==='entering'){s.entryTime+=dt;s.y=Math.max(115,s.y-dt*20);tick(s,dt);if(s.entryTime>=2.7){enterHall();return;}}else{let dx=Number(keys.has('d')||keys.has('arrowright'))-Number(keys.has('a')||keys.has('arrowleft')),dy=Number(keys.has('s')||keys.has('arrowdown'))-Number(keys.has('w')||keys.has('arrowup'));
  ...
  move(s,dx,dy,dt);tick(s,dt);ravenSound.update(s.ravens);campaign.elapsed+=dt;
