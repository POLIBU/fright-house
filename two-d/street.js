...
import {newCampaign,remember,once,openDoor,checkpoint,saveCampaign,loadCampaign} from './campaign/state.js';
import {STORY} from './campaign/areas.js';
const $=q=>document.querySelector(q),canvas=$('#game'),ctx=canvas.getContext('2d');ctx.imageSmoothingEnabled=false;
const bg=new Image();bg.src='./assets/street-clean.png';
...
const sound={ambience:new Audio('../audio/ambience.mp3'),mechanism:new Audio('../audio/wall.mp3')};sound.ambience.loop=true;sound.ambience.volume=.2;sound.mechanism.volume=.22;const resumeSounds=new Set();
function play(id){if(muted||paused)return;const a=sound[id];if(id!=='ambience')a.currentTime=0;a.play().catch(()=>{});}
function stop(){resumeSounds.clear();for(const a of Object.values(sound)){a.pause();a.currentTime=0;}}
...
function dismiss(){if(paused)return;dialog=null;$('#dialog').hidden=true;}
function pause(){if(!running)return;paused=!paused;campaign.paused=paused;release();$('#pause').textContent=paused?'RESUME':'PAUSE';if(paused){for(const [id,a]of Object.entries(sound)){if(!a.paused)resumeSounds.add(id);a.pause();}}else{for(const id of resumeSounds)if(!muted)sound[id].play().catch(()=>{});resumeSounds.clear();}}
...
function finish(){running=false;s.phase='complete';once(campaign,'street-complete');stop();$('#stage').classList.add('completed');$('#veil').hidden=false;$('#veil h1').textContent='THE DOOR IS OPEN';$('#veil .eyebrow').textContent='LEVEL 1 COMPLETE';$('#intro').innerHTML='Beyond the turnstile, the ticket hall is silent.<br>The next level will be built after its concept review.';$('#start').textContent='REPLAY LEVEL 1';$('#resume').hidden=true;}
function interact(){if(!running||paused)return;if(dialog){dismiss();return;}const o=near(s);if(!o)return;release();
  if(o.id==='newspaper'){s.newspaper=true;remember(campaign,'park-newspaper','PARK CLOSED AFTER CHILDREN VANISH — a witness saw lights inside after midnight. The clipping is dated October 19, 1987.');say(NEWSPAPER.title+' · '+NEWSPAPER.date,NEWSPAPER.body+'\n\n'+NEWSPAPER.clue+'\n\nClipping added to your field notes.','paper',NEWSPAPER.headline);}
  if(o.id==='notices'){s.notices=true;remember(campaign,'missing-notices','Three missing-child notices name Fright House as the last known location.');say('MISSING · THREE CHILDREN','Three photographs. Three families waiting for news.\n\nEach notice names the same place: FRIGHT HOUSE. The posters are much newer than the weeds around the gate.');}
  if(o.id==='ticket'){s.ticket=true;remember(campaign,'street-ticket','An admission ticket stamped OCT 17, 1987 lies outside the supposedly closed park.');say('AN ADMISSION TICKET','OCT 17, 1987. ADMIT ONE.\n\nThe stamp is recent. According to the newspaper, this attraction should have been closed.');}
  if(o.id==='stall'){s.stall=true;remember(campaign,'shuttered-stall','The prize stall is locked. Something rattles behind its shutter, but nobody answers.');play('mechanism');say('BEHIND THE SHUTTER','A soft rattle from inside. You wait.\n\nNo footsteps. No answer.');}
...
canvas.onpointerdown=e=>{if(!running||paused||dialog)return;const r=canvas.getBoundingClientRect(),p={x:(e.clientX-r.left)*480/r.width,y:(e.clientY-r.top)*360/r.height};
  ...
  if(p.x>221&&p.x<257&&p.y>112&&p.y<153)target=s.gate&&p.y<140?null:INTERACTIONS[4];
  const aim=target||p;route=path(s,aim.x,aim.y);pending=target?.id||null;if(target&&near(s)?.id===target.id)interact();
};
const bulbs=[[174,24,'#e45231'],[199,17,'#edb356'],[246,27,'#e35938'],[272,20,'#c6a25c'],[304,28,'#448bdf'],[47,173,'#d7402e'],[30,179,'#d5a744'],[80,167,'#39a66a'],[144,184,'#d54632'],[329,184,'#d49c43'],[348,190,'#397fd5'],[166,286,'#d9ab55'],[316,286,'#d9ab55'],[152,91,'#d9a452'],[327,86,'#d49a43']];
...
function paper(){if(s.newspaper)return;ctx.save();ctx.translate(63,242);ctx.rotate(-.25);rect(-8,-4,18,11,'#392e21aa');rect(-9,-5,18,10,'#c0ac79');rect(-8,-4,16,2,'#4c4030');line(0,-1,0,4,'#776243');for(let j=0;j<3;j++){rect(-7,-1+j*2,6,1,'#796647');rect(2,-1+j*2,5,1,'#796647');}ctx.restore();}
function gate(){const a=-.14+s.gateProgress*1.3;line(235,141,235+Math.cos(a)*12,141-Math.sin(a)*9,'#8a7957',2);rect(234,139,2,7,'#75684f');}
...
function render(){ctx.clearRect(0,0,480,360);if(ready)ctx.drawImage(bg,0,0,480,360);ferris();lighting();paper();gate();if(running||s.phase==='complete'){drawPlayer(ctx,s,(keys.size||route.length)&&!paused&&!dialog);flashlight();}fogLayer();
  ...
  $('#count').textContent=`FIELD NOTES ${campaign.journal.length}/4`;$('#objective').textContent=s.gate?'Walk through the broken turnstile into the funhouse.':s.newspaper?'Someone saw lights after closing. Find the entrance.':'The newspaper on the bench may explain why the park closed.';
}
function frame(now){const dt=Math.min(.04,(now-last)/1000);last=now;if(running&&!paused&&!dialog){let dx=Number(keys.has('d')||keys.has('arrowright'))-Number(keys.has('a')||keys.has('arrowleft')),dy=Number(keys.has('s')||keys.has('arrowdown'))-Number(keys.has('w')||keys.has('arrowup'));
  if(!dx&&!dy&&route.length){dx=route[0].x-s.x;dy=route[0].y-s.y;if(Math.hypot(dx,dy)<2){route.shift();dx=dy=0;}}
  move(s,dx,dy,dt);tick(s,dt);campaign.elapsed+=dt;
  if(!route.length&&pending){if(near(s)?.id===pending)interact();pending=null;}
  if(s.gateProgress>=1&&s.y<138){persist();finish();}
}render();requestAnimationFrame(frame);}
bg.onload=()=>{ready=true;$('#start').disabled=false;$('#start').textContent='BEGIN INVESTIGATION';$('#resume').hidden=!saved();};bg.onerror=()=>{$('#start').textContent='BACKGROUND FAILED — RELOAD';};
Object.defineProperty(window,'fright2d',{get:()=>JSON.parse(JSON.stringify({...s,area:'street',running,paused,dialog:dialog?.name||null,ready,near:near(s)?.id||null,route:route.length,notes:campaign.journal.map(n=>n.id),wheelAngle:wheelAngle(s.time*(reduced?.3:1)),bulbs:bulbs.map((_,i)=>bulbLevel(s.time,i,reduced)),audio:Object.fromEntries(Object.entries(sound).map(([id,a])=>[id,{paused:a.paused,time:a.currentTime}]))}))});requestAnimationFrame(frame);
