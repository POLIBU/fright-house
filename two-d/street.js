...
function start(resume=false){if(!ready)return;if(!resume&&!running){resetEvidence();resetInstructions();}inspector.close();toyInspector.close();stop();const previous=resume?saved():null;s=previous?.street||newStreet();s.light=true;confetti=newConfetti();campaign=previous?.campaign||newCampaign();campaign.paused=false;campaign.story=null;running=true;paused=false;dialog=null;release();$('#dialog').hidden=true;$('#veil').hidden=true;$('#stage').classList.remove('completed');$('#pause').textContent='PAUSE';canvas.focus({preventScroll:true});ravenSound.unlock();play('music');persist();}
...
}}render();drawSecretsOverlay();requestAnimationFrame(frame);}
bg.onload=()=>{forecourt=createForecourt(bg);ready=true;$('#start').disabled=false;$('#start').textContent='BEGIN INVESTIGATION';const canResume=!!saved()||hallSave();$('#start').hidden=canResume;$('#resume').hidden=!canResume;};bg.onerror=()=>{$('#start').textContent='BACKGROUND FAILED — RELOAD';};
