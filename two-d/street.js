...
Object.defineProperty(window,'fright2d',{get:()=>JSON.parse(JSON.stringify({...s,area:'street',running,paused,dialog:dialog?.name||null,inspection:inspector.snapshot(),toyInspection:toyInspector.snapshot(),props3d:props.snapshot(),foreground:forecourt?.snapshot(),ready,near:near(s)?.id||null,route:route.length,notes:campaign.journal.map(n=>n.id),wheelAngle:wheelAngle(s.time*(reduced?.3:1)),bulbs:bulbs.map((_,i)=>bulbLevel(s.time,i,reduced)),audio:Object.fromEntries(Object.entries(sound).map(([id,a])=>[id,{paused:a.paused,time:a.currentTime}]))}))});requestAnimationFrame(frame);
