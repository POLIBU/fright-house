...
function finish(){s.phase='entering';s.entryTime=0;release();play('mechanism');once(campaign,'street-complete');}
function enterHall(){s.phase='complete';enterDoor(campaign,'street-hall');campaign.paused=false;campaign.story=null;persist();stop();location.href='./level-2.html?arrival=mouth';}
...
function render(){ctx.clearRect(0,0,480,360);if(ready)ctx.drawImage(bg,0,0,480,360);ferris();forecourt?.draw(ctx,s,false);lighting();paper();props.draw(ctx,'ticket');props.draw(ctx,'toy');if(s.y>=150)props.draw(ctx,'gate',s.gateProgress);if(running||s.phase==='complete'){ctx.save();if(s.phase==='entering')ctx.globalAlpha=Math.max(0,1-s.entryTime/.65);drawPlayer(ctx,s,(keys.size||route.length)&&!paused&&!dialog);if(s.phase!=='entering')flashlight();ctx.restore();}forecourt?.draw(ctx,s,true);if(s.phase==='entering'){props.draw(ctx,'mouth',mouthProgress(s.entryTime));}props.drawRavens(ctx,s.ravens);if(s.y<150)props.draw(ctx,'gate',s.gateProgress);fogLayer();
  ...
  $('#count').textContent=`FIELD NOTES ${campaign.journal.length}/5`;$('#objective').textContent=s.phase==='entering'?'The mouth closes behind you…':s.gate?'Walk through the broken turnstile into the funhouse.':s.newspaper?'Someone saw lights after closing. Find the entrance.':'The newspaper on the bench may explain why the park closed.';
}
