import {requestGameFrame,configureTelemetry,resetTelemetryMotion,whenAssetsReady,markRoomLoaded} from './runtime-telemetry.js';
...
function start(resume=false){resetTelemetryMotion();if(!ready)return;if(!resume&&!running){resetEvidence();resetInstructions();}inspector.close();toyInspector.close();stop();const previous=resume?saved():null;s=previous?.street||newStreet();s.light=true;confetti=newConfetti();campaign=previous?.campaign||newCampaign();campaign.paused=false;campaign.story=null;running=true;paused=false;dialog=null;release();$('#dialog').hidden=true;$('#veil').hidden=true;$('#stage').classList.remove('completed');$('#pause').textContent='PAUSE';canvas.focus({preventScroll:true});ravenSound.unlock();play('music');persist();}
...
function interact(targetId=null){if(!running||paused||s.phase==='entering')return;if(dialog){dismiss();return;}const o=typeof targetId==='string'?INTERACTIONS.find(o=>o.id===targetId&&Math.hypot(o.x-s.x,o.y-s.y)<o.r):near(s);if(!o)return;release();
  ...
  if(o.id==='toy'){s.toyExamined=true;dialog={name:'WIND-UP CAROUSEL',type:'3d'};toyInspector.open(s.toy&&readInstructions().includes('carousel'),'toy',false);}
  ...
  if(o.id==='entrance'){if(!readInstructions().includes('carousel')){say('THE TURNSTILE','You have not collected important evidence. Find it and come back.');return;}if(!s.gate){s.gate=true;openDoor(campaign,'street-hall');play('mechanism');}else if(s.gateProgress>=1){say('THE TURNSTILE','The broken arm gives way. Walk through the clown’s mouth.');}}
  persist();
}
$('#start').onclick=()=>start();$('#resume').onclick=()=>{if(hallSave())leaveRoom('./level-2.html?arrival=resume');else start(true);};$('#retry').onclick=()=>start();$('#pause').onclick=pause;$('#continue').onclick=dismiss;$('#action').onclick=interact;$('#journal').onclick=journal;$('#sound').onclick=()=>{muted=!muted;ravenSound.mute(muted);for(const a of Object.values(sound)){a.muted=muted;if(!muted&&running&&!paused&&a.loop)a.play().catch(()=>{});}$('#sound').textContent=muted?'SOUND OFF':'SOUND ON';$('#sound').setAttribute('aria-label',muted?'Unmute sound':'Mute sound');};
window.addEventListener('keydown',e=>{if(toyInspector.active){if(['e','E'].includes(e.key)&&!e.repeat){e.preventDefault();toyInspector.close();}return;}if(inspector.active){if(['e','E'].includes(e.key)&&!e.repeat){e.preventDefault();inspector.close();}return;}if(['ArrowUp','ArrowDown','ArrowLeft','ArrowRight',' '].includes(e.key))e.preventDefault();if(e.repeat)return;if(e.key==='Escape'){pause();return;}if(['e','E',' '].includes(e.key)){interact();return;}if(e.key.toLowerCase()==='j'){journal();return;}keys.add(e.key.toLowerCase());route=[];pending=null;});window.addEventListener('keyup',e=>keys.delete(e.key.toLowerCase()));
...
function paper(){ctx.save();ctx.translate(63,242);ctx.rotate(-.25);drawEvidence(ctx,{kind:'paper'},0,0,1,!s.newspaper,s.time);ctx.restore();}
