...
const props=createPropRenderer();
...
const toyInspector=createToyInspector({onClose(){dialog=null;release();},onDiscover(){s.toy=true;remember(campaign,'windup-mark','A wind-up carousel concealed a brass plate marked 03: maintenance access, upper attraction.');persist();}});
...
function start(resume=false){if(!ready)return;inspector.close();toyInspector.close();stop();const previous=resume?saved():null;s=previous?.street||newStreet();campaign=previous?.campaign||newCampaign();campaign.paused=false;campaign.story=null;running=true;paused=false;dialog=null;release();$('#dialog').hidden=true;$('#veil').hidden=true;$('#stage').classList.remove('completed');$('#pause').textContent='PAUSE';canvas.focus({preventScroll:true});play('ambience');persist();}
...
function interact(targetId=null){if(!running||paused)return;if(dialog){dismiss();return;}const o=typeof targetId==='string'?INTERACTIONS.find(o=>o.id===targetId&&Math.hypot(o.x-s.x,o.y-s.y)<o.r):near(s);if(!o)return;release();
  ...
  if(o.id==='notices'){s.notices=true;remember(campaign,'missing-notices','Three missing-child notices name Fright House as the last known location.');inspect('notices','MISSING · THREE CHILDREN');}
  if(o.id==='ticket'){s.ticket=true;remember(campaign,'street-ticket','An admission ticket stamped OCT 17, 1987 lies outside the supposedly closed park.');inspect('ticket','AN ADMISSION TICKET');}
  if(o.id==='toy'){dialog={name:'WIND-UP CAROUSEL',type:'3d'};toyInspector.open(s.toy);}
...
canvas.onpointerdown=e=>{if(!running||paused||dialog)return;const r=canvas.getBoundingClientRect(),p={x:(e.clientX-r.left)*480/r.width,y:(e.clientY-r.top)*360/r.height};
  ...
  if(p.x>73&&p.x<93&&p.y>229&&p.y<248)target=INTERACTIONS.find(o=>o.id==='toy');
...
function render(){ctx.clearRect(0,0,480,360);if(ready)ctx.drawImage(bg,0,0,480,360);ferris();lighting();paper();props.draw(ctx,'toy');if(s.y>=150)props.draw(ctx,'gate',s.gateProgress);if(running||s.phase==='complete'){drawPlayer(ctx,s,(keys.size||route.length)&&!paused&&!dialog);flashlight();}if(s.y<150)props.draw(ctx,'gate',s.gateProgress);fogLayer();
  ...
  $('#count').textContent=`FIELD NOTES ${campaign.journal.length}/5`;$('#objective').textContent=s.gate?'Walk through the broken turnstile into the funhouse.':s.newspaper?'Someone saw lights after closing. Find the entrance.':'The newspaper on the bench may explain why the park closed.';
}
...
Object.defineProperty(window,'fright2d',{get:()=>JSON.parse(JSON.stringify({...s,area:'street',running,paused,dialog:dialog?.name||null,inspection:inspector.snapshot(),toyInspection:toyInspector.snapshot(),props3d:props.snapshot(),ready,near:near(s)?.id||null,route:route.length,notes:campaign.journal.map(n=>n.id),wheelAngle:wheelAngle(s.time*(reduced?.3:1)),bulbs:bulbs.map((_,i)=>bulbLevel(s.time,i,reduced)),audio:Object.fromEntries(Object.entries(sound).map(([id,a])=>[id,{paused:a.paused,time:a.currentTime}]))}))});requestAnimationFrame(frame);
